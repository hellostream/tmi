defmodule TwitchChat.Twitch.Client do
  @moduledoc """
  Twitch API.
  """
  ...
  def create_subscription(type, user_id, session_id, client_id, access_token)
    ...
    case resp do
      {:ok, %{status: 202, headers: _headers, body: body}} ->
        Logger.debug("[TwitchChat.Twitch.Client] #{type} subscription created:\n#{inspect(body)}")
        {:ok, body}

      {:ok, %{status: 429, headers: %{"ratelimit-reset" => resets_at}}} ->
        Logger.warning("[TwitchChat.Twitch] rate-limited; resets at #{resets_at}")
        {:error, resp}

      {:ok, %{status: _status} = resp} ->
        Logger.error("[TwitchChat.Twitch] unexpected response: #{inspect(resp)}")
        {:error, resp}

      {:error, error} ->
        Logger.error("[TwitchChat.Twitch] error making resquest: #{inspect(error)}")
        {:error, error}
    end
  end
