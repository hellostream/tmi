defmodule TwitchChat.EventSub.Socket do
  @moduledoc false
  use WebSockex
  ...
  defp do_start(opts) do
    Logger.info("[TwitchChat.EventSub.Socket] connecting...")
  ...
  def handle_frame({:text, msg}, state) do
    case Jason.decode(msg) do
      ...
      _ ->
        Logger.warning("[TwitchChat.EventSub.Socket] Unhandled message: #{msg}")
        {:ok, state}
    end
  end
  ...
  def handle_frame({type, msg}, state) do
    Logger.debug(
      "[TwitchChat.EventSub.Socket] unhandled message type: #{inspect(type)}, msg: #{inspect(msg)}"
    )
  ...
  def handle_cast({:send, {type, msg} = frame}, state) do
    Logger.debug("[TwitchChat.EventSub.Socket] sending #{type} frame with payload: #{msg}")
    {:reply, frame, state}
  end
  ...
  def terminate(close_reason, _state) do
    Logger.error("[TwitchChat.EventSub.Socket] terminating #{inspect(close_reason)}")
  end
  ...
  defp handle_message(%{"message_type" => "session_welcome"}, payload, state) do
    Logger.info("[TwitchChat.EventSub.Socket] connected")
  ...
  defp handle_message(%{"message_type" => "session_keepalive"}, _payload, _state) do
    Logger.info("[TwitchChat.EventSub.Socket] keepalive")
  end
  ...
  defp handle_message(
    ...
    Logger.debug(
      "[TwitchChat.EventSub.Socket] got notification: " <> inspect(payload, pretty: true)
    )

    type
    |> TwitchChat.EventSub.Events.from_payload(payload)
    |> add_delayed_event()
  ...
  defp handle_message(%{"message_type" => "session_reconnect"}, _payload, _state) do
    Logger.debug("[TwitchChat.EventSub.Socket] reconnect message")
  end
  ...
  defp handle_message(%{"message_type" => "revocation"}, payload, _state) do
    Logger.error("[TwitchChat.EventSub.Socket] sub revoked: #{inspect(payload)}")
  end
  ...
  defp handle_message(_metadata, payload, _state) do
    # TODO: match ^
    Logger.error("[TwitchChat.EventSub.Socket] closed: #{inspect(payload)}")
  end
  ...
  defp add_delayed_event(%TwitchChat.EventSub.Events.AdBreakBegin{} = event) do
    ad_break_end = struct(TwitchChat.EventSub.Events.AdBreakEnd, Map.from_struct(event))
    Process.send_after(self(), {:delayed_event, ad_break_end}, event.duration_seconds * 1000)
    event
  end

  defp add_delayed_event(%TwitchChat.EventSub.Events.ShoutoutCreate{} = event) do
    cooldown_end = struct(TwitchChat.EventSub.Events.ShoutoutCooldownEnd, Map.from_struct(event))

    cooldown_target_end =
      struct(TwitchChat.EventSub.Events.ShoutoutCooldownTargetEnd, Map.from_struct(event))
