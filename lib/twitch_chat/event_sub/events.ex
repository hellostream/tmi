defmodule TwitchChat.EventSub.Events do
  @moduledoc false
  require Logger

  @events %{
    "channel.ad_break.begin" => TwitchChat.EventSub.Events.AdBreakBegin,
    "channel.follow" => TwitchChat.EventSub.Events.Follow,
    "channel.shoutout.create" => TwitchChat.EventSub.Events.ShoutoutCreate,
    "channel.shoutout.receive" => TwitchChat.EventSub.Events.ShoutoutReceive
  }

  # @event_names Map.keys(@events)

  # Generate the AST for all the module's struct types as a union
  # like `TwitchChat.EventSub.Events.Follow.t() | TwitchChat.EventSub.Events.ShoutoutReceive` etc...
  event_types =
    Map.values(@events)
    |> Enum.sort()
    |> Enum.reduce(&{:|, [], [{{:., [], [&1, :t]}, [], []}, &2]})
  ...
  ## Examples
      ...
      %TwitchChat.EventSub.Events.Follow{
  ...
  defp payload_map({key, val}) do
    Logger.warning("""
    [TwitchChat.EventSub.Events] You have found an unexpected field: #{inspect({key, val})}.
  ...
  defp parse_datetime(val) do
    case DateTime.from_iso8601(val) do
      ...
      {:ok, datetime, offset} ->
        Logger.error("[TwitchChat.EventSub.Events] unexpected offset (#{offset}) parsing #{val}")
        datetime
